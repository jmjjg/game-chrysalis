...
GameResultsPanel.prototype.write = function(games) {
	...
	var game, settings, row, link;

	$('#game-results-panel tbody tr').remove();

	$.each(games, function(idx){
		game = games[idx];
		settings = game.events[0].settings;
		//glyphicon-search
		link = '<a href="#" onclick="game.view(\'' + game.start + '\');return false;"><span class="glyphicon glyphicon-search"></span> Voir</a>';
